//! Period bucketing and per-period aggregation.
//!
//! Turns a time column into named period labels ("2024-03", "2024-W11", …) at a
//! chosen granularity, then aggregates a metric per period. Labels are strings
//! on purpose: they are what the report shows, and carrying them alongside the
//! numbers avoids re-deriving a display form from a timestamp later.
...
use crate::stats::significance::{mean, std_dev};
