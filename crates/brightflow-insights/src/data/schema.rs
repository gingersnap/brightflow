...
use crate::data::config::{SchemaConfig, TimeGranularity};
...
impl DataSchema {
    /// Create schema from a config file
    pub fn from_config(config: &SchemaConfig) -> Self {
        let time_columns = config.time_columns();
        ...
        Self {
            ...
            time_columns,
            time_column,
            time_granularity: config.settings.time_granularity,
        }
    }
