...
/**
 * Extraction tab: the extract function, the vocabularies it resolves
 * against (feedback categories, products, competitors), the mention table
 * it materialised, and the unresolved-subject queue — names the extractor
 * used that match no product or competitor. Everything the extract call
 * reads or writes, and nothing the classify call does.
 */

import { useQuery, useQueryCache } from '@pinia/colada';
import { computed, ref } from 'vue';

import { enrichFnApi, mentionsApi, taxonomyApi, vocabularyApi } from '@/services/api';
...
<template>
  <div class="flex flex-col gap-8">
    <FunctionCard kind="ticket_extract" :source-id="sourceId" :table="table" />

    <VocabularyTree
      :source-id="sourceId"
      :table="table"
      :kinds="['feedback_category', 'product', 'competitor']"
    />
